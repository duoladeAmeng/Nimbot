...
class _ExecSession:
    def __init__(
        ...
        *,
        ...
        self._deadline_task = asyncio.create_task(self._watch_deadline()) if timeout else None

    async def _watch_deadline(self) -> None:
        try:
            await asyncio.wait_for(self.process.wait(), max(0, self.deadline - time.monotonic()))
        except TimeoutError:
    ...
    async def poll(
        ...
        *,
        ...
        if self.process.returncode is not None:
            ...
            from duolaAgent.agent.tools.shell import (  # pyright: ignore[reportPrivateUsage]
                ExecTool,
...
class WriteStdinTool(Tool):
    ...
    @classmethod
    def config_cls(cls):
        from duolaAgent.agent.tools.shell import ExecToolConfig
