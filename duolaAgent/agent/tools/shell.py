...
from duolaAgent.agent.tools.base import Tool, ToolResult, tool_parameters
from duolaAgent.agent.tools.context import ToolContext, current_request_session_key
...
from duolaAgent.agent.tools.sandbox import wrap_command
...
from duolaAgent.config.paths import get_media_dir
...
from duolaAgent.security.workspace_policy import is_path_within
...
class ExecTool(Tool):
    ...
    def _wrap_path_export(self, command: str, env: dict[str, str]) -> str:
        segments: list[str] = []
        if self.path_prepend:
            env["DUOLA_PATH_PREPEND"] = self.path_prepend
            segments.append("$DUOLA_PATH_PREPEND")
        segments.append("$PATH")
        if self.path_append:
            env["DUOLA_PATH_APPEND"] = self.path_append
            segments.append("$DUOLA_PATH_APPEND")
    ...
    def _create_windows_job() -> _ProcessTreeOwner:
        from duolaAgent.agent.tools._windows_job import WindowsJob
    ...
    def _guard_command(
        ...
        *,
        ...
        from duolaAgent.security.network import contains_internal_url
