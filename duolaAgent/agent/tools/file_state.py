...
def record_read(path: str | Path, offset: int = 1, limit: int | None = None) -> None:
    current_file_states(_default).record_read(path, offset=offset, limit=limit)


def record_write(path: str | Path) -> None:
    current_file_states(_default).record_write(path)


def check_read(path: str | Path) -> str | None:
    return current_file_states(_default).check_read(path)


def is_unchanged(path: str | Path, offset: int = 1, limit: int | None = None) -> bool:
    return current_file_states(_default).is_unchanged(path, offset=offset, limit=limit)
